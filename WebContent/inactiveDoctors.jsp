...
%>
		 ...
		 var out = "";
			...
			for(i = 0; i < data.length; i++) {
				...
				if(data[i].mode == "INACTIVE"){
     			...
     			'<td>' + data[i].email + '</td>' +
     			'<td>'+'<button type="button" class="btn btn-primary" onclick="mode(\'' + data[i].doctor_id + '\')">Activate</button>' +
				'</td>'+
	        	'</tr>';
	...
	function mode(doct_id){
		...
		var deleteDoctor = confirm('Do you want to Activate this doctor?');
		if(deleteDoctor){
			
		$('#e'+doct_id).hide();
		setMode("ACTIVE",doct_id);
...
<body class="bgcolor" oncontextmenu="return false">
	...
	<div class="row" style="padding: 20px 100px;">
		...
		<h2>List of Inactive Doctors&emsp;&emsp;
		&emsp;&emsp;</h2>
