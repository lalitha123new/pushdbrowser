               		...
               		<div class="col-md-6">
                		<div>
							...
							<div class="panel-body">
								...
								<hr>
								<div class="form-group row">
									<label class="col-sm-6 text-right"><strong>I am ready</strong></label>
									<div class="col-sm-6 ">
										<a id="start" type="button" onclick="" 
	...
	//	F7464A
		...
		$(document).ready(function() {
			 ...
			 $.ajax({
				  ...
				  success: function(data){
						...
						//new navigation logic
						 if(activeSubSectionId != 0) { // resume where user left off
							 
							   if(activeSectionId < 11) {
							       // Redirected to current last visited EssentialZone Section  (same as activeSection?);
							       window.location.replace("section2.jsp");
								   $('#start').attr("onclick", "getNextSubSection("+sectionId+", "+subsectionId+")");
							       
							   }else {
							        // Redirected to current last visited OptionalZone Section (same as activeSection?)
							       window.location.replace("section3.jsp");
								   $('#start').attr("onclick", "getNextSubSection3("+sectionId+", "+subsectionId+")"); 
							  }
						 
							} else {  // about to visit a new section
							  if(sectionId < 11) {  // move to next unvisited Essential Section
							      // Redirected to unvisited EssentialZone Section
							     $('#start').attr("onclick", "getNextSubSection("+sectionId+", "+subsectionId+")");
							  } else {    // let user decide which section they want to visit
							      // Display new page with all Section list. 
							      window.location.replace("landing_page.jsp");
										  ...
										  success: function(data1){
											  ...
											  if(data2 !=null){
													...
													for(var p = 0;p<data2.length;p++){
														...
														else if(total_exercises > 3 && total_exercises <=19 )
														$('#fb2').removeClass("hidden");
														else if(total_exercises > 19 && total_exercises < 26 )
			...
			function myFunction(arr) {
				...
				var section_id_new = sectionId;
				if(section_id_new > 10){
					
					section_id_new = section_id_new-10;
				}
				
				var out = '<ul class="list-unstyled">';
	    	    for(i = 0; i < arr.length; i++) {
	    	    	out +=  '<li>&emsp;' + section_id_new + '.' + (i+1) + '&emsp;' + 
			    				'<i>' + arr[i] + '</i>' + 
			    			'</li>';
	    	    }
	    	    out += '</ul>';
