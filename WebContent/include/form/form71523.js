...
Vromansys.Payment = new function() {
	...
	return {
		...
		initPayPalExpress: function(key, environment) {
			paypal.checkout.setup(key, {
				environment: environment,
				container: 'button-paypal-express',
				button: 'button-paypal-express',
				click: function(event) {
					...
					$.support.cors = true;
  					...
  					var url = $('#PayPalSetExpressCheckoutUrl').val();
					$("#PaymentType").val("9");
					$.ajax({
						dataType: "json",
						type: "POST",
						...
						success: function (response) {
							if(response.status == "success") {
								...
								$(".payment-option").css("visibility", "hidden");
								$('#button-paypal-express').parent().css("visibility", "visible");
								var height = $('#button-paypal-express').height();
								$('#button-paypal-express').html('<img src="/images/icons/loading/ajax-loader-big.gif" alt="loading" />');
								$('#button-paypal-express').height(height);
								$('#button-paypal-express').off('click');
							} else if(response.status == "error") {
								var errorMsgs = '';
								$.each(response.messages, function(idx, errorMsg) {
									if(errorMsgs) {
										errorMsgs += "<br />";
									}
									errorMsgs += errorMsg;
								...
								alert("Unknown status: " + response);
							}
						},
						error: function (responseData, textStatus, errorThrown) {
							try {
								paypal.checkout.closeFlow();
								var responseJson = jQuery.parseJSON(responseData.responseText.substring(0, responseData.responseText.indexOf("<!DOCTYPE")));
								alert("Error sending to PayPal: " + responseJson.messages);
							} catch (e) {
								alert("Error sending to PayPal: " + errorThrown);
							}
						}
					});
				}
			...
			// process error redirects
			var sPageURL = window.location.search.substring(1);
			var sURLVariables = sPageURL.split('&');
			for (var i = 0; i < sURLVariables.length; i++) {
				var sParameterName = sURLVariables[i].split('=');
				if (sParameterName[0] == "RedirectErrorToken") {
					paypal.checkout.initXO();
					$.support.cors = true;
					paypal.checkout.startFlow(sParameterName[1]);
